...
class EvidenceService:
    _FEATURE_CANDIDATES = _FEATURE_CANDIDATES
    ...
    def build_evidence_chain(self, alert: Alert) -> EvidenceChainSchema:
        ...
        if not top_features:
            top_features = self._extract_features_from_flows(evidence.get("flow_ids", []))
    ...
    def _extract_features_from_flows(self, flow_ids: list[str]) -> list[dict]:
        """Rank candidate features by max absolute value across the given flows."""
        if not flow_ids:
            return []

        from app.models.flow import Flow

        flows = self.db.query(Flow).filter(Flow.id.in_(flow_ids[:10])).all()
        if not flows:
            return []
        ...
        for flow in flows:
            ...
            for name in _FEATURE_CANDIDATES:
