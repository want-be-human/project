  ...
  return (
    <div className="bg-white rounded-lg shadow border border-gray-200">
      <div className="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
        ...
        <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">{t('available', { count: scenarios.length })}</span>
      </div>
        ...
        {scenarios.map((s) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 mb-1">
              <div>
                <span className="font-semibold text-gray-400">{t('minAlerts')}</span> {s.expectations?.min_alerts || 0}
              </div>
              <div>
                <span className="font-semibold text-gray-400">{t('dryRun')}</span> {s.expectations?.dry_run_required ? t('required') : t('skipped')}
              </div>
            </div>
            <div className="text-xs text-gray-500">
              <span className="font-semibold text-gray-400">{t('pcap')}</span> <span className="font-mono text-[10px]">{s.pcap_ref?.pcap_id?.slice(0,18)}...</span>
            </div>
          </div>
        ))}
      </div>
