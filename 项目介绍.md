...
- [一、数据存储与持久化](#一数据存储与持久化)
  ...
  - [1.5 PostgreSQL 迁移性能调优](#15-postgresql-迁移性能调优从-oom-崩溃到-158-秒)
- [二、数据接入](#二数据接入)
...
graph TB
    ...
    classDef engine fill:#FFEBEE,stroke:#E53935,stroke-width:2px,color:#B71C1C
    ...
    %% 用户层
    subgraph UserGroup [用户层]
        Browser["浏览器 (Next.js + React 19 + TypeScript)"]
    end
    class UserGroup,Browser user

    %% 前端服务
    subgraph FrontendGroup [前端服务 Port 3000]
        NextJS["Next.js 16 App Router - SSR / 静态生成"]
        UI_Dashboard["仪表盘 - ECharts / React Three Fiber"]
        UI_Alerts["告警管理"]
        UI_Flows["流量浏览"]
        UI_Topology["拓扑可视化 - ReactFlow / Three.js"]
        UI_PCAP["PCAP 管理 - 上传 / 处理进度"]
        UI_Batch["批量处理"]
        UI_Scenario["场景测试"]
        WS_Client["WebSocket 客户端 - 实时事件推送"]
    end
    class FrontendGroup,NextJS,UI_Dashboard,UI_Alerts,UI_Flows,UI_Topology,UI_PCAP,UI_Batch,UI_Scenario,WS_Client frontend

    %% 后端服务
    subgraph BackendGroup [后端服务 Port 8000]
        FastAPI["FastAPI REST API - Pydantic 校验"]
        WS_Server["WebSocket 推送 - EventBus -> ConnectionManager"]

        subgraph BusinessLayer [业务服务层]
            Parsing["PCAP 解析服务 - NFStream / dpkt"]
            Features["特征工程服务 - 18+ 维统计特征"]
            Detection["复合检测服务"]
            Alerting["告警聚合服务"]
            Agent["智能调查服务 - 分诊 -> 调查 -> 建议"]
            Twin["数字孪生服务 - 行动计划 / 模拟推演"]
            Decision["决策建议服务 - 三级推荐框架"]
            Analytics["分析与评分服务 - 安全态势 / 行动安全"]
            BatchSvc["批处理服务 - 多文件编排"]
        end

        subgraph DetectionEngine [三层检测引擎]
            L1["Layer 1: 基线检测 - IsolationForest (scikit-learn)"]
            L2["Layer 2: 规则增强 - 启发式规则 + 端口/协议匹配"]
            L3_Graph["图特征构建 - 中心性 / 社区检测"]
            L3["Layer 3: 监督排序 - XGBoost 分类器"]
        end

        subgraph CoreInfra [核心基础设施]
            EventBus["事件总线 - 异步发布/订阅"]
            DB_Layer["SQLAlchemy 2.0 ORM"]
            Config["配置管理"]
            Observability["可观测性 - OpenTelemetry + TensorBoardX"]
            Logging["结构化日志"]
        end
    end
    class BackendGroup,FastAPI,WS_Server backend
    class BusinessLayer,Parsing,Features,Detection,Alerting,Agent,Twin,Decision,Analytics,BatchSvc backend
    class CoreInfra,EventBus,DB_Layer,Config,Observability,Logging backend
    class DetectionEngine,L1,L2,L3_Graph,L3 engine

    %% 数据层
    subgraph DataGroup [数据层]
        PostgreSQL[("PostgreSQL 15 - pcap_files / flows / alerts / twin_plans / dryrun_executions / batches / scenarios")]
        FileStorage[("文件存储 - PCAP 文件 / ML 模型 - Docker Named Volume")]
    end
    class DataGroup,PostgreSQL,FileStorage data

    %% 基础设施
    subgraph InfraGroup [基础设施]
        Docker["Docker 容器化"]
        Compose["Docker Compose 编排"]
        Alembic["Alembic 数据库迁移"]
    end
    class InfraGroup,Docker,Compose,Alembic infra
...
flowchart TB
    ...
    classDef layer1 fill:#E8F5E9,stroke:#43A047,stroke-width:2px,color:#1B5E20
    classDef layer2 fill:#FFF3E0,stroke:#FB8C00,stroke-width:2px,color:#E65100
    classDef layer3 fill:#F3E5F5,stroke:#8E24AA,stroke-width:2px,color:#4A148C
    ...
    %% 并行检测层 (L1, L2a, L2b)
    subgraph DetectionLayers [多层检测引擎]
        direction LR
        L1["Layer 1: IsolationForest<br/><b>无监督</b><br/>预训练于 CICIDS2017<br/>→ baseline_score"]
        L2a["Layer 2a: 规则增强<br/><b>13 维</b><br/>端口扫描 / DoS / 暴力破解<br/>→ rule_score"]
        L2b["Layer 2b: 图特征构建<br/><b>12 维</b><br/>度/中心性/关联检测<br/>→ graph_score"]
    end
    class L1 layer1
    class L2a,L2b layer2

    %% 融合与决策层
    subgraph FusionLayer [融合与决策]
        L3["Layer 3: XGBoost<br/><b>62 维特征融合</b><br/>→ final_score + label"]
        Guardrails["防护栏 (Guardrails)<br/>强规则保底 + 多源共识"]
    end
    class L3 layer3
    class Guardrails layer3
...
graph LR
    ...
    classDef volume fill:#FFF3E0,stroke:#FB8C00,stroke-width:2px,color:#E65100,shape:cylinder
    classDef subgraphStyle fill:#F5F5F5,stroke:#BDBDBD,stroke-width:2px

    subgraph DockerCompose [🐳 Docker Compose]
        direction LR
        FE["frontend<br/><b>Port 3000</b><br/>Next.js 16"]
        BE["backend<br/><b>Port 8000</b><br/>FastAPI + Uvicorn"]
        DB[("db<br/><b>Port 15432→5432</b><br/>PostgreSQL 15")]
...
- 上传一个8G的pcap文件时，花费一个多小时也无法结束，后端进程直接阻塞超时
...
### 1.5 PostgreSQL 迁移性能调优：从 OOM 崩溃到 158 秒

从 SQLite 迁移到 PostgreSQL 后，系统在处理 8GB PCAP 文件（CIC-IDS2017 Friday 数据集，501K 条流记录）时遭遇了严重的性能回退——从 SQLite 优化版的 390 秒退化到超过 600 秒甚至 OOM 崩溃。经过四个阶段的系统性诊断与优化，最终将端到端处理时间压缩至 **158 秒**。

#### 第一阶段：诊断——为什么 PostgreSQL 比优化后的 SQLite 慢

```mermaid
flowchart TB
    Problem["PostgreSQL 初始版本<br/>8GB PCAP → >600s + OOM 崩溃"]

    subgraph 根因分析
        P1["PG 零调优<br/>shared_buffers=128MB<br/>synchronous_commit=on"]
        P2["COPY + 5 索引同时维护<br/>501K × 5 B-tree<br/>= 250 万次索引插入"]
        P3["逐条 Alert INSERT<br/>每条 5.5ms 网络往返"]
        P4["BULK_SIZE 未定义<br/>NameError 崩溃"]
    end

    Problem --> P1 & P2 & P3 & P4
    
    style Problem fill:#f87171,color:#fff
    style P1 fill:#fbbf24,color:#000
    style P2 fill:#fbbf24,color:#000
    style P3 fill:#fbbf24,color:#000
    style P4 fill:#fbbf24,color:#000
```

| 问题 | 根因 | 影响 |
|------|------|------|
| PostgreSQL 零调优 | `shared_buffers=128MB`（默认值），`synchronous_commit=on` | COPY 501K 行时脏页频繁刷盘 + 每笔事务 fsync |
| COPY + 5 索引同时维护 | 501K 行 × 5 个 B-tree 索引 = 250 万次索引插入 | 写入后半段指数级减速（"卡死"现象） |
| 逐条 Alert INSERT | 每条 alert 一次网络往返 + fsync | 数千 alert × 5.5ms ≈ 数十秒 |
| BULK_SIZE 未定义 | `stages.py` 迁移 COPY 时删除了变量定义 | batch 路径 NameError 崩溃 |

> **关键洞察**：SQLite+WAL 优化版虽然写入慢，但零网络开销（嵌入式）；PostgreSQL 的优势（MVCC、COPY 协议）需要配合正确的服务端参数才能发挥，默认配置反而比调优后的 SQLite 更慢。

#### 第二阶段：PostgreSQL 调优 + 写入优化

| 优化项 | 技术手段 | 原理 | 效果 |
|--------|---------|------|------|
| PG 服务端调优 | `synchronous_commit=off`, `shared_buffers=256MB`, `wal_buffers=16MB`, `max_wal_size=2GB` | 关闭同步提交避免每事务 fsync；增大缓冲区减少脏页抖动；增大 WAL 缓冲减少 checkpoint 频率 | 等效 SQLite `synchronous=NORMAL` 的写入吞吐 |
| Drop/Rebuild Index | COPY 前移除 5 个索引 → COPY 批量写入 → 一次性 `CREATE INDEX` | 避免逐行 B-tree 维护；PostgreSQL 一次性构建索引使用排序算法，比逐条插入快 10-50 倍 | COPY 501K 行仅 19 秒（含索引重建） |
| Alert 批量 INSERT | 逐条 `db.add()` → 收集后 50K 批次 `db.execute(insert(), alert_dicts)` | 消除 N 次网络往返，合并为单次 executemany | 数千 alert 写入从数十秒降至 <1 秒 |
| 流式 COPY | `_FlowTSVStream` 迭代器替代 `StringIO` 整体缓冲 | 不再将全部 TSV 数据拼接到内存中，而是按需生成每行 | 峰值内存减少约 500MB |

```mermaid
graph LR
    A["COPY + 5 索引<br/>501K 行<br/>指数级减速"] -->|"Drop Index → COPY → Rebuild"| B["COPY 无索引<br/>501K 行 19s<br/>线性写入"]
...
Docker Desktop for Windows 使用 WSL2 后端，默认内存上限约 7.8GB。PostgreSQL 容器 + Python 后端 + Docker shm 的内存总和超过了这一限制。

| 问题 | 根因 | 修复方案 |
|------|------|---------|
| Docker 容器 OOM 崩溃 | WSL2 仅 7.8GB 可用，PG 512MB + Python ~3GB + shm 1GB 超限 | PG `shared_buffers` 降至 256MB，`shm_size` 设为 512MB，移除 `wal_level=minimal` 等激进参数 |
| 后台任务丢失卡"聚合中" | OOM 杀进程 → 容器重启 → 任务状态永久卡住 | 内存参数优化后不再触发 OOM |

> **教训**：在资源受限的容器环境中，数据库参数不能照搬生产服务器的"最优配置"。`shared_buffers=256MB` + `synchronous_commit=off` 在 WSL2 环境下是最佳平衡点。

#### 第四阶段：CPU 瓶颈修复

数据库写入优化后，CPU 成为新的瓶颈——Layer 3 XGBoost 推断从预期的 30 秒膨胀到 212.7 秒。

| 问题 | 根因 | 修复方案 |
|------|------|---------|
| Layer 3 XGBoost 推断 212.7s | `feature_importances_` 是 `@property`（每次访问解析 200 棵树的 JSON 配置），被 501K 次循环调用 | `_precompute_top_features()` 在推断前预计算一次特征重要性，循环内用缓存值 |
| XGBoost GPU 降级警告 | 模型文件含 `device=cuda` 设置，Docker 容器无 GPU | `_force_cpu_if_needed()` 在模型加载时检测环境 |

```python
# 修复前：每条流都触发 @property，解析 200 棵树
for flow in flows:  # 501K 次
    importances = model.feature_importances_  # 每次 O(200棵树) 解析

# 修复后：预计算一次
top_features = self._precompute_top_features()  # 仅一次
for flow in flows:
    # 直接使用预计算结果，O(1) 查找
```
...
```mermaid
graph LR
    Stage1[("阶段一：SQLite 原始<br><br>🔧 使用技术<br>SQLite 单文件<br>库级写锁 / 逐条 fsync<br>ORM 逐条 INSERT<br><br>⏱ 处理耗时<br>50 万条写入 → 数十分钟<br>8GB PCAP → >1 小时（阻塞超时）")]
    Stage2[("阶段二：SQLite+WAL + 写入与落盘优化<br><br>🔧 使用技术<br>SQLite + WAL 模式<br>PRAGMA 调优（64MB / NORMAL）<br>Core Bulk Insert + 单次 Commit<br>Docker Named Volume（ext4 原生速度）<br><br>⏱ 处理耗时<br>50 万条写入 → 约 30 秒<br>8GB PCAP → 约 390 秒")]
    Stage3[("阶段三：PostgreSQL 初始（未调优）<br><br>🔧 使用技术<br>PostgreSQL 15<br>COPY 协议 + 5 索引同时维护<br>逐条 Alert INSERT / 零调优<br><br>⏱ 处理耗时<br>8GB PCAP → >600 秒 + OOM 崩溃")]
    Stage4[("阶段四：PostgreSQL 最终优化<br><br>🔧 使用技术<br>PG 调优（buffers=256MB, sync=off）<br>Drop/Rebuild 索引<br>Alert 批量插入 / 流式 COPY<br>预计算特征<br><br>⏱ 处理耗时<br>8GB PCAP → 158 秒<br>（COPY 50 万条 → 19 秒）")]

    Stage1 --> Stage2 --> Stage3 --> Stage4

    style Stage1 fill:#f8d7da,stroke:#b71c1c,stroke-width:2px,color:#721c24
    style Stage2 fill:#fff3cd,stroke:#f0ad4e,stroke-width:2px,color:#856404
    style Stage3 fill:#ffe5d0,stroke:#e67e22,stroke-width:2px,color:#a04000
    style Stage4 fill:#d4edda,stroke:#2e7d32,stroke-width:2px,color:#155724
```

| 指标 | SQLite 优化版 | PG 初始版 | PG 最终版 | 提升 |
|------|-------------|----------|----------|------|
| **总耗时 (8GB Friday)** | ~390s | >600s + OOM 崩溃 | **158s** | 2.5× vs SQLite，3.8× vs PG 初始 |
| **DB 持久化** | ~120s | 卡死 | **49s** | 2.4× vs SQLite |
| **Layer 3 推断** | ~30s (估) | 212.7s | **39s** | 5.5× vs PG 初始 |
| **内存稳定性** | 稳定 | OOM 崩溃 | **稳定** | — |
| **50 万条 COPY** | N/A (bulk insert) | 卡死（索引维护） | **19s** | — |

> 四个阶段的优化本质上对应了四种不同的性能瓶颈：**I/O 瓶颈**（PG 参数调优）→ **索引瓶颈**（Drop/Rebuild 策略）→ **内存瓶颈**（OOM 修复）→ **CPU 瓶颈**（预计算缓存）。这说明大数据处理中的性能优化往往不是单一维度的，需要逐层剥离瓶颈。
